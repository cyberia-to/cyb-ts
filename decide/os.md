...
```
1. UEFI loads nox binary. Ring 0.
2. hal.storage → load genesis BBG
3. verify BBG root (O(1))
4. soma initializes: read energy, sigma, triggers
5. soma dispatches pending Orders
6. loop: perceive → decide → act → learn
```

## self-hosting

everything above metal boundary is a [[nox]] formula.
...
evaluator IS a nox formula — metacircular interpreter. jets: optimization, not architecture. remove all jets → works, slower.
...
| [[soma]] metabolism | yes | nox formula in trace |
| hal dispatch logic | yes | nox formula in trace |
| device read | no | hardware witness |
| device write | no | physics ≠ math |
...
|--|------|-------|----------|-------|
| ISA | ~1500 | 13 | ~140 | 18 |
| state | mutable | event log | Merkle | polynomial |
| state proof | none | none | O(log n) | O(1) |
| crash | journal | full replay | re-sync | load root |
| I/O | ~400 syscalls | ~20 effects | ~10 precompiles | call+look |
| resources | OOM killer | none | gas (1D) | 4D + energy |
| [[survival]] | none | none | none | sigma + bounty |
| provability | none | none | per-tx | per-step |
| metal | millions LOC | ~50K | N/A | ~10 LOC |
| self-hosting | no | partial | no | yes |

see [[cyb/stack]] for the crates this kernel is built from. see [[cyb/features]] for the capabilities it provides. see [[cyb/survival]] for the machine survival protocol. see [[cyb/soma]] for the machine mind. see [[cyb/order]] for the execution unit.